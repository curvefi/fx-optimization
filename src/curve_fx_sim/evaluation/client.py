...
class SubprocessHarnessClient(HarnessClient):
    """Adapter translating orchestrator specs to the sibling canonical client."""
    ...
    def __init__(
        ...
        *,
        ...
        self.root = repository.resolve() if repository is not None else Path.cwd().resolve()
    ...
    def _write_session_manifest(
        ...
        scenario_spec: ScenarioSpec,
        *,
        market_files: Sequence[dict[str, object]],
    ) -> Path:
        path = Path(self._session_manifest_dir.name) / f"{scenario_spec.scenario_fingerprint()}.json"
        return atomic_write_json(
            path,
            {
                "schema_version": "fxsim_manifest_v1",
                "run_kind": "session",
                "run_id": f"session-{scenario_spec.id}",
                "resolved_spec": {
    ...
    def open_session(
        ...
        self.prepare()
        if scenario_spec.template_path is None:
            raise FileNotFoundError("scenario template_path is required by the harness protocol")
        template = (self.root / scenario_spec.template_path).resolve()
        if not template.is_file():
            raise FileNotFoundError(f"template file not found: {template}")
        market_files: list[dict[str, object]] = []
        for market in scenario_spec.market_files:
            path = (self.root / market.path).resolve()
            if not path.is_file():
                raise FileNotFoundError(f"market file not found: {path}")
            digest = sha256_path(path)
            if market.sha256 and market.sha256 != digest:
                raise ValueError(
                    f"market file hash mismatch for {market.path}: "
                    f"expected {market.sha256}, calculated {digest}"
                )
            market_files.append(
                {
                    "path": path.as_posix(),
        ...
        manifest = self._write_session_manifest(
            scenario_spec,
            market_files=market_files,
        )
        ready = self._client.open_session(
            session_id=sess_id,
            template_path=template,
            manifest_path=manifest,
            template_sha256=scenario_spec.template_sha256 or sha256_path(template),
            manifest_sha256=sha256_path(manifest),
            **scenario_spec.harness_session_config(),
        )
