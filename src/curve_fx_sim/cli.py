...
@click.option("--arb", "arb_path", type=click.Path(exists=True, file_okay=False, path_type=Path), default=None,
              help="Collected fxsim run directory (legacy --arb alias for run_dir)")
@click.option("--metrics", default=None, help="Comma-separated metrics (default: first available legacy defaults)")
@click.option("--cmap", default="turbo", help="Colormap (default: turbo)")
@click.option("--max-ticks", type=int, default=12)
@click.option("--ncol", type=int, default=3, help="Tiles per row")
@click.option("--clamp", is_flag=True, default=False, help="Clamp negative values in color limits")
@click.option("--skewthr", "--pricethr", "--imbalancethr", type=float, default=0.0,
              help="Max max_7d_skew percent for masked metrics; 0 defaults to the data max.")
@click.option("--max-pricethr", type=float, default=100.0, help="Max 7d price-diff threshold in bps")
@click.option("--slipthr", type=float, default=20.0, help="Initial slippage cap in bps")
@click.option("--slipthr-max", type=float, default=100.0, help="Maximum slippage-slider value in bps")
@click.option("--last-pdifthr", type=float, default=0.0,
              help="Max final_rel_price_diff in bps for masked metrics; 0 defaults to the data max.")
@click.option("--log-axis", multiple=True, help="Plot axis on a log scale (repeatable / comma-separated)")
@click.option("--start-time", default=None, help="Legacy inspect start override")
@click.option("--harness", type=click.Path(exists=True, file_okay=True, path_type=Path), default=None,
...
def view_run(
    ...
    """Open the legacy-style N-D heatmap explorer for one collected run.

    Separate Heatmaps/Controls/Metrics windows with X/Y radio buttons,
    per-dimension and threshold sliders, masked metrics, hover readout, and
    shift/right-click exact-cell replay through the unified harness.
    """
    try:
        from .plotting.viewer import open_viewer

        target = run_dir if run_dir is not None else arb_path
        if target is None:
            raise click.ClickException("a run directory is required (positional or --arb)")
        exit_code = open_viewer(
            target,
            metrics=[m.strip() for m in metrics.split(",") if m.strip()] if metrics else None,
            ncol=ncol,
            cmap=cmap,
            max_ticks=max_ticks,
            clamp=clamp,
            ...
            last_pdifthr=last_pdifthr,
            log_axes=list(log_axis),
            start_time=start_time,
            out=out,
            harness=str(harness) if harness else None,
        )
        if exit_code != 0:
            raise click.ClickException(f"viewer exited with code {exit_code}")
    except Exception as exc:  # noqa: BLE001
        _fail(exc)
