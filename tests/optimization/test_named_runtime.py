import hashlib, json; from contextlib import nullcontext
...
METRICS = {"apy_net": .05, "apy_net_gm": .04, "avg_rel_price_diff": .005, "detach_energy_ungated": .001,
           "duration_s": 86400., "max_7d_rel_price_diff": .02,
           "trades": 100, "tw_real_slippage_1pct": .0005}
...
def _compiler():
    parameters = [
        ...
        _d("run.session_id", "open_session.session_id", "string", "identifier", "utf8"), _d("run.template_path", "open_session.template_path", "string", "path", "utf8"),
        _d("run.template_sha256", "open_session.template_sha256", "string", "sha256", "lower_hex_64"), _d("run.manifest_path", "open_session.manifest_path", "string", "path", "utf8"),
        _d("run.manifest_sha256", "open_session.manifest_sha256", "string", "sha256", "lower_hex_64"), _d("run.pool_index", "open_session.pool_index", "integer", "count", "uint64", default=0),
        _d("run.n_candles", "open_session.n_candles", "integer", "count", "uint64", default=10), _d("run.yb_mode", "open_session.yb_mode", "enum", "yb_mode", "utf8", default="off", choices=["off", "passive", "active_2l"]),
        _d("run.yb_releverage", "open_session.yb_releverage", "boolean", "legacy_alias", "json_boolean", default=False),
        _d("run.metric_projection", "evaluate_batch.metric_projection", "enum", "projection", "utf8", "observation", choices=["summary", "full"]),
    ]
    schema = {"schema_version": "curve_fx_parameter_schema_v1", "parameters": parameters}
    canonical = canonical_json_bytes(schema).decode(); return CandidateCompiler.from_description({
        "schema_version": "curve_fx_evaluator_description_v1", "policy": {"id": "compiled_named", "parameter_count": 1, "descriptor_abi_version": 1},
        "parameter_schema": schema, "parameter_schema_canonical_json": canonical,
...
class _Selected(SelectedEvaluator):
    def __init__(self, root, compiler, publish=True):
        ...
        identity = {"binary_sha256": binary_sha, "harness_version": "1", "pool_version": "1", "policy_id": "compiled_named",
            "policy_source_sha256": "c" * 64, "policy_abi": "twocrypto_policy_v1", "policy_parameter_count": 1,
            "compiler": "test", "numeric_mode": "double", "real_type": "double",
            "build_target": "test", "ipo_enabled": False, "native_tuning": False}
        hello = {"protocol": "curve_fx_eval_v1", "type": "hello", "version": 1, "evaluator_identity": identity, "metric_fields": list(METRICS)}
        provenance = {"schema_version": "curve_fx_selected_evaluator_v1", "artifact_sha256": "a" * 64, "build_spec_sha256": "b" * 64, "binary_sha256": binary_sha, "parameter_schema_sha256": compiler.schema.sha256,
            "policy": {"id": "compiled_named", "abi": "twocrypto_policy_v1", "source_sha256": "c" * 64}}
        values = {"artifact": SimpleNamespace(receipt_path=receipt), "compiler": compiler, "_verified_payload_json": canonical_json_bytes(hello).decode(), "binary_path": binary,
            "binary_sha256": binary_sha, "artifact_sha256": "a" * 64, "build_spec_sha256": "b" * 64, "parameter_schema_sha256": compiler.schema.sha256,
            "provenance_json": canonical_json_bytes(provenance).decode()}
...
class _Client:
    ...
    def open_session(self, **request):
        ...
        return SessionReadyFrame(request_id="o", session_id=self.session_id, scenarios=[], scenario_set_sha256="1" * 64,
...
def setup(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(ScenarioSpec, "harness_session_config", lambda self:
                        {"n_candles": self.n_candles, "yb_mode": self.yb_mode,
    ...
    SITE = SiteProfile(name="cluster", site_type="ssh", cluster=ClusterConfig(coordinator="b1", transport="shared_nfs",
        remote_base=PurePosixPath(str(tmp_path)),
        remote_run_root=PurePosixPath(str(tmp_path / "runs")), repository_root=PurePosixPath(str(tmp_path)),
        worker_command="/bin/fxsim", blades=("b1", "b2")))
    ...
    _Optimizer.tells = 0
    spec = OptimizationSpec(id="named", pair_id="pair", policy_id="compiled_named", algorithm="nevergrad_two_points_de", scenarios=("s1", "s2"),
        parameter_space={"policy.weight": {"min": 0., "max": 2., "step": .5},
                         "pool.out_fee": {"min": .001, "max": .003, "step": .001},
                         "run.n_candles": [10, 20]},
        optimizer_config={"budget": 2, "batch_size": 2, "seed": 1})
...
def test_named_batch_groups_numeric_sessions_and_preserves_order(setup, tmp_path, monkeypatch):
    ...
    def dispatch(**kwargs):
        calls.append(kwargs)
        results = {row.evaluation_id: CandidateResult(ordinal=row.ordinal, candidate_id=row.evaluation_id, economic_fingerprint="f" * 64,
            metrics=METRICS) for row in kwargs["evaluations"]}
    ...
    result = run_optimization(spec, store=store, repository=tmp_path, site="cluster",
                              blades=("b1", "b2"), selected_evaluator=SELECTED)
    ...
    assert (calls[0]["request_namespace"], calls[0]["lane_count"]) == ("opt_000000", 1)
    payload = execution_site_payload(SITE, ("b1", "b2"), artifact_selected=True)
...
def test_named_resume_replays_prefix_and_rejects_changed_lineage(setup, tmp_path):
    store, spec = setup
    first = run_optimization(spec, store=store, repository=tmp_path, run_id="resume", budget=1,
                             batch_size=1, selected_evaluator=SELECTED)
    run_dir = first.manifest_path.parent
    prefix = (run_dir / "evaluation_journal.jsonl").read_text().splitlines()[0]
    for name in ("manifest.json", "evaluation_table.npz", "winner.json", "topk.json"):
        (run_dir / name).unlink()
    run_optimization(spec, store=store, repository=tmp_path, run_id="resume", resume=True,
                     selected_evaluator=SELECTED)
    assert (run_dir / "evaluation_journal.jsonl").read_text().splitlines()[0] == prefix
    checkpoint = json.loads((run_dir / "checkpoint.json").read_text())
    checkpoint["evaluation_lineage"][0][0]["session_group_id"] = "0" * 64
    (run_dir / "checkpoint.json").write_text(json.dumps(checkpoint))
    with pytest.raises(ValueError, match="committed evaluation lineage changed"):
        run_optimization(spec, store=store, repository=tmp_path, run_id="resume", resume=True,
                         selected_evaluator=SELECTED)
