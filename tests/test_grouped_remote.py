...
from curve_fx_sim.execution.grouped_remote import (GridWorkRequest, GroupedRemoteError,
    GroupedWorkReceipt, GroupedWorkRequest, execute_grid_work, execute_grouped_work)
...
from curve_fx_sim.artifacts.manifest import load_manifest
...
def test_worker_roundtrip_and_tamper(tmp_path: Path, monkeypatch):
    repo = tmp_path / "repo"; repo.mkdir(); scenario, evaluations, _, selected, hello = _fixture(repo, 5)
    run_root = tmp_path / "runs"; run = run_root / "run"; (run / "evaluator_artifact").mkdir(parents=True)
    request = GroupedWorkRequest("run", "request", evaluations, (scenario,), canonical_json_bytes(selected.provenance), 1, 3,
    ...
    launches = []; monkeypatch.setattr("curve_fx_sim.execution.grouped.EvaluatorClient", lambda *args, **kwargs: launches.append(kwargs["launch_argv"]) or _Client(hello))
    ...
    receipt = execute_grouped_work(path, run / "grouped_receipts/request.json", remote_run_root=run_root, repository=repo,
        blade="blade-b1")
    assert loaded == [run / "evaluator_artifact"] and [x.candidate_id for x in receipt.results] == [f"e{i}" for i in range(5)]
    assert json.loads(request.canonical_json)["evaluator_workers"] == 3 and "lane_count" not in json.loads(request.canonical_json)
    assert launches == [[selected.binary_path, "serve", "--workers", "3"]]
...
def test_two_blade_dispatch_rejects_duplicates_and_attestation_drift(tmp_path: Path):
    ...
    assert dispatch_grouped_evaluations(run_root=site.cluster.remote_run_root / "run", run_id="run", selected=selected,
        ...
        chunk_size=1, evaluator_workers=1, request_namespace="attempt_0001", ssh=SSH()).requests == 2
    for value in ("duplicate", "drift"):
        ...
        with pytest.raises(GroupedRemoteError): dispatch_grouped_evaluations(run_root=site.cluster.remote_run_root / "run", run_id="run",
            ...
            repository=repo, site=site, chunk_size=1, evaluator_workers=1, request_namespace="attempt_0001", ssh=SSH())
def _site(root):
    base = root / "remote"; return SiteProfile("blades", "ssh", cluster=ClusterConfig(coordinator="blade-b1", transport="shared_nfs",
        ...
        harness=HarnessConfig(chunk_size=1), runner=RunnerConfig(max_workers=2, worker_concurrency=3))
def test_backend_grouped_cluster_resume_sends_no_completed_work(tmp_path: Path, monkeypatch):
    repo = tmp_path / "repo"; repo.mkdir(); scenario, raw, groups, selected, hello = _fixture(repo, 5); site = _site(tmp_path)
    evaluations = tuple(replace(item, evaluation_id=f"grid_g_p{i:06d}") for i, item in enumerate(raw))
    ...
    authority = {"schema_version": "fxsim_cartesian_grid_v1", "grid_id": "g", "axes": [[{"display": {"x": i}, "proposal": {}} for i in range(5)]],
        "static_proposal": {}, "artifact_sha256": selected.artifact_sha256, "parameter_schema_sha256": selected.parameter_schema_sha256,
        "scenario_key": {"sha256": evaluations[0].candidate.scenario_key.sha256, "identity_json": evaluations[0].candidate.scenario_key.identity_json.decode()},
        "baseline_session_key": {"sha256": evaluations[0].candidate.session_key.sha256, "identity_json": evaluations[0].candidate.session_key.identity_json.decode()},
        "baseline_observation_key": {"sha256": evaluations[0].observation_key.sha256, "identity_json": evaluations[0].observation_key.identity_json.decode()},
        "coordinate_shape": [5], "pool_count": 5}
    plan_sha = hashlib.sha256(canonical_json_bytes(authority)).hexdigest(); plan_record = authority | {"plan_sha256": plan_sha}
    plan = SimpleNamespace(plan_sha256=plan_sha, pool_count=5, artifact_sha256=selected.artifact_sha256,
        candidate_id_at=lambda ordinal: f"grid_g_p{ordinal:06d}",
        iter_points=lambda ranges: (SimpleNamespace(evaluation=evaluations[i], session_group_id=groups[0].key.sha256)
            for start, end in ranges for i in range(start, end)))
    projection = MetricProjection(("score",), "grid").to_dict()
    resolved = {"scenario": scenario.to_dict(), "policy": {"id": "p", "source_sha256": "d"*64, "policy_abi": "v1"},
        "grid": {"id": "g"}, "metric_projection": projection, "evaluator_artifact_selection": selected.provenance}
    manifest = new_grid_manifest(run_id="run", grid_id="g", pool_count=5, resolved_spec=resolved, plan=plan_record,
    ...
    path = run / "manifest.json"; path.write_bytes(canonical_json_bytes(manifest))
    loads = []; monkeypatch.setattr(SelectedEvaluator, "load", lambda value: loads.append(Path(value)) or selected)
    plan_loads = []; monkeypatch.setattr("curve_fx_sim.grids.runner.load_grid_plan", lambda *a, **k: plan_loads.append(1) or plan)
    monkeypatch.setattr("curve_fx_sim.execution.backend._preflight", lambda p, r: SimpleNamespace(
        manifest=load_manifest(p, expected_kind="grid"), selected=selected, scenario=scenario, plan=plan))
    launches = []; monkeypatch.setattr("curve_fx_sim.execution.grouped_grid.EvaluatorClient",
        lambda *a, **kw: launches.append(kw["launch_argv"]) or _Client(hello))
    process = _BackendProcess(repo, site.cluster.remote_run_root)
    backend = ExecutionBackend(site_profile=site, process_adapter=process)
    first = backend.run_grid(path, repository=repo, chunk_size=2, blades=("blade-b1",))
    request = process.requests[0]; payload = json.loads(request.canonical_json)
    representative = replace(request, shards=tuple({"shard_id": f"shard_{i:06d}", "shard_index": i,
        "ranges": [[i*2048, (i+1)*2048]]} for i in range(32))).validated()
    second = backend.run_grid(path, repository=repo, chunk_size=2, blades=("blade-b1",), resume=True)
    assert first.executed_shards == 3 and second.skipped_shards == 3 and process.workers == 1
    assert len(request.canonical_json) < 2048 and len(representative.canonical_json) < 8192
    assert not ({"evaluations", "scenarios", "coordinates", "results"} & payload.keys())
    assert loads == [run / "evaluator_artifact"] * 3 and plan_loads == [1, 1, 1]
    assert launches == [[selected.binary_path, "serve", "--workers", "3"]]
    assert set(json.loads((run / "grouped_receipts" / f"{request.request_id}.json").read_text())) == {
        "schema_version", "status", "request_sha256", "run_id", "request_id", "blade",
        "plan_sha256", "artifact_sha256", "shards", "elapsed_seconds"}
    assert {item.name for item in (run / "results").iterdir()} == {
        *(f"shard_{i:06d}.npz" for i in range(3)),
        *(f"shard_{i:06d}.receipt.json" for i in range(3)),
    } and not (run / "grid_results.npz").exists()
class _BackendProcess:
    def __init__(self, repo, run_root): self.repo, self.run_root, self.workers, self.requests = repo, run_root, 0, []
    def run(self, argv, **kwargs):
        command = argv[-1]
        assert "/attack" not in command
        if " worker grid " in f" {command} ":
            self.workers += 1; parts = shlex.split(command); request_path = Path(parts[parts.index("grid") + 1])
            request = GridWorkRequest.from_json(request_path); self.requests.append(request)
            execute_grid_work(request_path, Path(parts[parts.index("--out") + 1]), remote_run_root=self.run_root,
                repository=self.repo, blade=parts[parts.index("--blade") + 1])
